...
function getStock() { try { const s = localStorage.getItem('pac_stock'); if (!s) return {...stockInicial}; const p = JSON.parse(s); const m = {...stockInicial}; Object.keys(p).forEach(k => { m[k] = (p[k]===false||p[k]===0) ? 0 : (typeof p[k]==="number" ? p[k] : (m[k] ?? 0)); }); return m; } catch(e) { return {...stockInicial}; } }
